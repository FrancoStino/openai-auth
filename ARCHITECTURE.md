...
- Contains: `getSettings`, `getConfigDir`, `getConfigPath`, `DEFAULT_CODEX_API_ENDPOINT`; leveled logger with redaction (Bearer/sk-/JWT, secret/api-key/password/token-like keys), 5 MiB log rotation keeping 3 generations; request-dump writer with redaction for `authorization`/`chatgpt-account-id`/`cookie`/`set-cookie`, embedding `process.pid` in filenames to avoid collisions across processes and restarts, recovering diff baselines from existing disk dumps across restarts, and diffing on redacted body text so offsets match `.body.json`.
